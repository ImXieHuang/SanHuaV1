...
def dot(v1: Vector, v2: Vector) -> Union[float, fraction]:
    _validate_vectors(v1, v2)
    result = sum(x*y for x,y in zip(v1.components, v2.components))
    if all(isinstance(x, fraction) for x in v1.components) and all(isinstance(x, fraction) for x in v2.components):
...
def abs_vector(v: Vector):
    ...
    for i in v.components:
        if isinstance(i, fraction):
            abs_list.append(fraction(abs(i.numerator), abs(i.denominator)))
        else:
            abs_list.append(abs(i))
    return Vector(abs_list)

def compare(v1: Vector, v2: Vector) -> Union[float, fraction]:
    diff_vector = sub(v1, v2)
    abs_diff = abs_vector(diff_vector)
    
    if all(isinstance(x, fraction) for x in abs_diff.components):
        result = fraction(1, 1)
