...
def FusionCCATransformer(CAT1: CCA.CCATransformer, CAT2: CCA.CCATransformer) -> CCA.CCATransformer:
    ...
    return CCA.CCATransformer(FusionDatabsase, CAT1.temperature/2 + CAT2.temperature/2)
...
def get_meaning_of_tokens_for_(CAT: CCA.CCATransformer, tokens: list[str]) -> list[Vector]:
    ...
    for i in range(len(tokens)):
        ...
        max_val = max(row)
        exp_row = [CCA.Const.E.value ** (val - max_val) for val in row]
        sum_exp = sum(exp_row)
    ...
    for i in range(len(tokens)):
        for j in range(i+1):
            if i == 0:
                bigQ_graph[i][j] = CAT.get_key_for_(tokens[i])
            else:
                vector_list = []
                for k in range(j):
                    components = []
    ...
    meaning_vectors[0] = CAT.get_key_for_(tokens[0])
...
def get_meaning_of_sentence_for_(CAT: CCA.CCATransformer, tokens: list[str]) -> Vector:
    tokens_meaning = get_meaning_of_tokens_for_(CAT, tokens)
    ...
    for i in range(len(tokens)):
        row = influence_graph[i]
        max_val = max(row)
        exp_row = [CCA.Const.E ** (val - max_val) for val in row]
        sum_exp = sum(exp_row)
        influence_graph[i] = [val / sum_exp for val in exp_row]
...
def get_meaning_of_sentence_at_(CAT: CCA.CCATransformer, AtQ: Vector, tokens: list[str]) -> Vector:
    tokens_meaning = get_meaning_of_tokens_at_(CAT, AtQ, tokens)
...
def think_about_next_token_for_(CAT: CCA.CCATransformer, tokens: list[str], T: float = CCA.Const.E.value) -> str:
    ...
    for token in CAT.get_tokens():
        ...
        exponent = dot_val / T
        weight = CCA.Const.E.value ** exponent
        cnt_n = Vector([i for i in mexp.add(cnt_n, mexp.mul(weight, CAT.get_key_for_(token))).components])
        cnt_m += weight
    ...
    return CAT.query_best_token_for_(next_token, sentence_meaning)
...
def get_complate_for_(CAT: CCA.CCATransformer, tokens: list[str]):
    sentence_meaning = get_meaning_of_sentence_for_(CAT, tokens[1:-1])
    token_meaning = get_meaning_of_tokens_for_(CAT, tokens)[-1]
