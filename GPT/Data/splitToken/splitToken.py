...
class ChineseTokenizer:
    ...
    def save_model(self, model_name=None):
        ...
        vocab_path = models_dir / f"{model_name}_vocab.txt"
        ...
        print(f"Vocab saved to: {vocab_path} ({len(self.tokens)} tokens)")
        return model_name

    def load_model(self, model_name):
        models_dir = self.get_models_dir()
        
        txt_path = models_dir / f"{model_name}_vocab.txt"
        if txt_path.exists():
            print(f"Loading vocab from txt: {txt_path}")
            ...
            if json_path.exists():
                with open(json_path, 'r', encoding='utf-8') as f:
                    info = json.load(f)
                    self.max_token_len = info.get('max_token_len', self.max_token_len)
                    self.min_freq_factor = info.get('min_freq_factor', self.min_freq_factor)
                    self.model_info = info.get('model_info', {})
            
            print(f"Loaded {len(self.tokens)} tokens from txt")
            return True
        
        model_path = models_dir / f"{model_name}.pkl"
        if not model_path.exists():
            available = [f.stem.replace('_vocab', '') for f in models_dir.glob("*_vocab.txt")]
            raise FileNotFoundError(f"Model '{model_name}' not found. Available: {available}")
        
        print(f"Loading from pickle: {model_path}")
        ...
        print(f"Loaded {len(self.tokens)} tokens from pickle")
        return True
    ...
    def list_models(self):
        ...
        for json_file in models_dir.glob("*_info.json"):
            try:
        ...
        for txt_file in models_dir.glob("*_vocab.txt"):
            model_name = txt_file.stem.replace('_vocab', '')
            if not any(m.get('model_name') == model_name for m in models):
                models.append({
                    'model_name': model_name,
                    'num_tokens': sum(1 for _ in open(txt_file, 'r', encoding='utf-8')),
                    'created_at': datetime.fromtimestamp(txt_file.stat().st_mtime).isoformat()
                })
    ...
    def tokenize(self, text, method='max_match'):
        if not self.tokens:
            raise ValueError("Please train or load model first")
